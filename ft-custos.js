// ft-custos.js — v3.1
// Feat 4: custo por fatia  Feat 7: comparação de receitas  Feat 9: overhead + mão de obra
// Fix v3.1: salvarConfig com debounce (era chamado a cada keypress sem debounce)
...
export function renderSimulador() {
    ...
    const opts = recs.length
        ? recs.map(r => `<option value="${r.id}">${_esc(r.nome)} (${r.tamanho})</option>`).join('')
        : '';
        ...
        comp.innerHTML = ings.length
            ? ings.map(ing => {
                ...
                return `<div class="ft-comp-row">
                    <span class="ft-comp-nome">${_esc(ing.nome)}</span>
...
function _renderComparar(recs) {
    ...
    const opts = recs.map(r => `<option value="${r.id}">${_esc(r.nome)} (${r.tamanho})</option>`).join('');
    return `
        ...
        resultEl.innerHTML = `<div class="ft-cmp-grid">
            ${ids.map(id => {
                ...
                <div class="ft-cmp-card">
                    <div class="ft-cmp-card-title">${_esc(r.nome)} <span class="ft-tam-pill">${r.tamanho}</span></div>
