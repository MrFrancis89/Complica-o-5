                ...
                <button class="ft-list-item" data-id="${i.id}" type="button">
                    ...
                    <span class="ft-item-body">
                        ...
                        <span class="ft-item-sub">
                            ${formatQtdUnid(i.quantidade_embalagem, i.unidade)} · ${formatCurrency(i.preco_compra)}${trend}
                        </span>
                    </span>
...
export function abrirFormIngrediente(id = null) {
    ...
    document.getElementById('_iDel')?.addEventListener('click',   async () => { fecharModal(null); await _del(id); });
    return done;
}
...
async function _del(id) {
    ...
    if (!ing) return;
    const ok = await confirmar(`Remover <strong>${_esc(ing.nome)}</strong>?<br>Não pode ser desfeito.`, { labelOK: 'Remover' });
    if (!ok) return;
